package stream.stepic.design_patterns.chain_of_responsability;
 ...
 * The first method is needed to use lambda expressions.
...
class ChainOfResponsibilityDemo {
    ...
    static RequestHandler commonRequestHandler = wrapInRequestTag
            .setSuccessor(createDigest
                    .setSuccessor(wrapInTransactionTag));

    //a combination of existing handlers here
    //wrapInTransactionTag, createDigest and wrapInRequestTag.
    ...
    interface RequestHandler {
        // method handle that accept request and returns new request here
        // it allows to use lambda expressions for creating handlers below
        Request handler(Request obj);

        //a default method for combining this and other handler single one
        ...
        default RequestHandler setSuccessor(RequestHandler requestHandler) {
            return (Request req) -> handler(requestHandler.handler(req));
        }
    }
     ...
     */
    static class Request { //XML requests in a payment system
    ...
    public static void main(String[] args) throws Exception {
        String str = "<type>payment</type><sum>100000</sum><order_id>e94dc619-6172-4ffe-aae8-63112c551570</order_id><desc>We'd like to buy an elephant</desc>";
        final Request notCompletedRequest = new Request(str);

        String data = commonRequestHandler.handler(notCompletedRequest).getData();
